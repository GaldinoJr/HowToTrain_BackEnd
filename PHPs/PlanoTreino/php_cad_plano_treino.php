<?php
    ...
    if(strcmp('SEND-json', $_POST['method']) == 0)
    {
        ...
        if(strcmp($operacao,"salvar") == 0)
        {
            ...
            if (!$result) 
            {
                $objPlanoTreino->status = -1; // Não conseguiu registrar o plano de treino, vazio
            ...
            else
            {
                ...
                {
                    $objPlanoTreino->status = -2; // Não conseguiu registrar o plano de treino, não é vazio, mas não tem conteudo
                }
                mysqli_close($connect);
            }
        ...
        else if(strcmp($operacao,"alterar") == 0)
        {
            ...
            $objPlanoTreino->objPlanoTreino->codigoExterno = $jsPlanoTreino->codigoExterno;
            
            $objJsPlanoTreino = $objJsPlanoTreino->planoTreino;
            ...
            $chamada_proc = "CALL " . $proc . " ('" .$xml . "', " . $objPlanoTreino->objPlanoTreino->codigoExterno . ")";
            // Execulta
            //echo $chamada_proc;
