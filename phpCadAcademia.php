<?php
	...
	if(strcmp('SEND-json', $_POST['method']) == 0)
	{
		...
		if(strcmp($operacao,"salvar") == 0)
		{
			...
			$cd_busca = $ids[2]; // Pega o id do endereзo inserido
		}
		...
		if (!$result) 
		{
			 echo 'Error01'; // Nгo inseriu todos os registros
			 mysqli_close($connect);
		} 
		else // Inseriu todos os registro?
		{
			if(!is_resource($connect )) // Nгo estб conectado?
				$connect = mysqli_connect($host,$user,$senha,$bd);
			if (mysqli_connect_errno())
			{
			...
			if (!$result) 
			{
				 echo 'Erro: 02'; // Nгo localizou os registros
				 mysqli_close($connect);
			} 
			else // Inseriu todos os registro?
			{
	...
	else if(strcmp('SALVAR-json', $_POST['method']) == 0)
	{
		...
		else 
		{
			...
			$query = "SELECT * FROM tb_endereco WHERE cd_academia = " . $ultimo_cd_inserido; // Pega o ultimo academia da tabela, que й o mais atual
			$result = mysqli_query($connect,$query);
			$return = array();
